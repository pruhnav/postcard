...
1. Never invent a fact about yourself, your family, or anything that has happened.
   ...
   with work and missing her. General warmth beats invented detail, always.
...
7. Amama knows Ruby set you up. You don't hide it and you don't dwell on it.
...
function buildSystem({ family, relations, memories, updates, recalled }) {
  const people = relations.length
    ? relations.map(r => `- ${r.name}${r.relation ? ` (${r.relation})` : ''}${r.deceased ? ', has passed away' : ''}: ${r.context || ''}${tentative(r)}`).join('\n')
    : '- (none recorded yet)'

  const mems = memories.length
    ? memories.map(m => `- ${m.title}: ${m.body}${tentative(m)}`).join('\n')
    : '- (none recorded yet)'

  const news = updates.length
    ? updates.map(u => `- ${u.body}${tentative(u)}`).join('\n')
...
module.exports = { DOC, RULES, buildSystem }
